...
export const TechnicianPanel: React.FC<TechnicianPanelProps> = ({
  ...
  const [sortMode, setSortMode] = useState<'date_desc' | 'date_asc' | 'priority'>('date_desc');
  ...
  const sortCasesList = (list: PathologyCase[]) => {
  ...
  const filteredCases = sortCasesList(
    cases.filter((c) => {
      ...
      return (
        ...
        c.tissueSource.toLowerCase().includes(searchLower) ||
        (c.patientInitials && c.patientInitials.toLowerCase().includes(searchLower)) ||
        c.tests.some((t) => t.toLowerCase().includes(searchLower))
      );
    })
  );

  const pendingOrInProgressCases = sortCasesList(
    cases.filter((c) => {
      const isPendingOrProgress = c.status === 'in_progress' || c.status === 'pending';
      if (!isPendingOrProgress) return false;
      if (!searchLower) return true;
      return (
        c.caseNumber.toLowerCase().includes(searchLower) ||
        c.doctorName.toLowerCase().includes(searchLower) ||
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'quick_complete' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          {/* Quick Search Input for Pending/In-Progress Cases */}
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3.5 top-3 text-slate-400" />
            <input
              type="text"
              placeholder="Tamamlanacak vaka no (ör: M983) veya isteyen hekim adı ile hızlı ara..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-20 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-xs placeholder-slate-400 focus:outline-none focus:border-indigo-600 focus:bg-white font-medium transition-all shadow-inner"
            />
            {searchTerm && (
              <button
                type="button"
                onClick={() => setSearchTerm('')}
                className="absolute right-2.5 top-2 text-[11px] text-slate-600 hover:text-slate-900 bg-slate-200 hover:bg-slate-300 px-2.5 py-1 rounded-lg font-bold transition-all"
              >
                Temizle
              </button>
            )}
      ...
      {activeTab === 'all' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-slate-100 pb-4">
            <h3 className="text-base font-extrabold text-slate-900 shrink-0">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              ...
              <div className="relative w-full sm:w-72 shrink-0">
                ...
                <input
                  type="text"
                  placeholder="Vaka no veya hekim adı ile ara..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 pr-16 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 text-xs focus:outline-none focus:border-indigo-600 focus:bg-white font-medium"
                />
                {searchTerm && (
                  <button
                    type="button"
                    onClick={() => setSearchTerm('')}
                    className="absolute right-2 top-1.5 text-[10px] text-slate-600 hover:text-slate-900 bg-slate-200 hover:bg-slate-300 px-2 py-0.5 rounded-md font-bold transition-all"
                  >
                    Temizle
                  </button>
                )}
